...
namespace AssetMonitoring.Web.Data
{
    public class UserProfileService : ICrud<UserProfile>
    {
        ...
        public List<UserProfile> GetSalesman()
        {
            var data = from x in db.UserProfiles.Where(x=>x.Role == Roles.Salesman)
                       select x;
            return data.ToList();
        } 
        
        public List<UserProfile> GetEngineer()
        {
            var data = from x in db.UserProfiles.Where(x=>x.Role == Roles.Workshop)
                       select x;
            return data.ToList();
        }
            ...
            try
            {
                ...
                foreach (var row in worksheet.Rows)
                {
                    if (counter > 0)
                    {
                        ...
                        newUserProfile.Username = string.IsNullOrEmpty(row.Cells[10].Value?.ToString().Trim()) ? MailHelper.GenerateEmailFromName(newUserProfile.FullName.Trim(), "AssetMonitoring.online") : row.Cells[10].Value?.ToString().Trim();
                        if (string.IsNullOrEmpty(newUserProfile.Email))
                        {
                            newUserProfile.Email = newUserProfile.Username;
                        }
                        newUserProfile.Role = row.Cells[11].Value?.ToString().Trim() == "Ya" ? Roles.Salesman : Roles.Admin;
        ...
        public byte[] ExportToExcel()
        {
            ...
            foreach (var item in datas)
            {
                ...
                worksheet.Cells[row, 8].Value = item.Role == Roles.Admin ? "Ya" : "Tidak";
